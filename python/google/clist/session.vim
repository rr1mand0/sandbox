...
nnoremap ,gd :GitDiff
map ,fl :call ToggleLocationList()
...
map ,, :!python % 2>&1 | tee /tmp/cfile:cfile /tmp/cfile
map ,ct :!rsync -av ~/shared/ -e ssh base:shared/
map ,cr :!rsync -av ~/src/aw/tools/linux_builder.sh -e ssh base:
map ,cx :!rsync -av ~/src/aw/chef/cookbooks/pxe -e ssh base:/var/chef/cookbooks/pxe
map ,cc :!rsync -av ~/tmp/chef_solo_deploy/ -e ssh xbase:/var/chef/
map ,l l
map ,h h
map ,k k
map ,j j
...
set shiftwidth=2
set softtabstop=2
set statusline=%m-%{g:host}@%f-[%n](%l/%L)--<<<%{GitBranch()}>>>
set tabstop=2
...
silent only
cd ~/sandbox/python/google/clist
...
badd +9 /tmp/cfile
badd +116 sample.py
badd +1 u\'updated\':\ u\'2013-02-03T05
args Task.py
edit Task.py
...
silent! normal! zE
let s:l = 96 - ((1 * winheight(0) + 13) / 27)
