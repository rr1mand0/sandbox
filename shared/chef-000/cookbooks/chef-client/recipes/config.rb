...
sslfiles = [
  node["chef_client"]["ssl_client_key"],
  node["chef_client"]["ssl_client_cert"],
  node["chef_client"]["ssl_ca_file"]
]
for sslfile in sslfiles do
  if sslfile 
    cookbook_file "#{node["chef_client"]["conf_dir"]}/#{sslfile}" do
      source "certs/#{sslfile}"
      owner "root"
      group root_group
      mode 0640
    end
