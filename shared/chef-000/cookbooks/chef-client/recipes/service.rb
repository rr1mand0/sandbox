...
when "bsd"
  ...
  when "freebsd"
    ...
    template "/etc/rc.d/chefclient" do
      source "#{dist_dir}/rc.d/chefclient.erb"
      mode 0755
      variables(
        :client_bin => client_bin
      )
      notifies :restart, "service[chefclient]", :delayed
    end

    #need to write 'chef-client_enable="YES"'  to /etc/rc.conf, huge ugly hack 
    #ruby_block 'edit /etc/rc.conf' do
    #  block do
    #    if ! File.open('/etc/rc.conf').lines.any? { |line| line.include?('chefclient_enable') }
    #      File.open('/etc/rc.conf', 'a') do |file|
    #        file.puts "chefclient_enable=\"YES\""
    #      end
    #    end
    #   end  #end block do
    #end #end ruby_block	
    ...
    template "/etc/rc.conf.d/chefclient" do
      source "#{dist_dir}/rc.conf.d/chefclient.erb"
      mode 0644
      variables(
        :enable_daemon => node["chef_client"]["enable_daemon"]
      )
      notifies :restart, "service[chefclient]", :delayed
    end
    ...
    if !node["chef_client"]["enable_daemon"]
      execute "stop_chef" do
        command "/etc/rc.d/chefclient onestop"
        action :run
      end
    end
  else
